...
from keras_custom.sampling import Sampling
...
def custom_builder(nframe=18, nagent=30, nhead=8, is_natural=False, debug_dict=None):
    ...
    def vae_sampling(sp1):
        sampling_input = keras.Input(shape=(nframe * nagent * nhead,))
        z_mean = layers.Dense(sp1, name='z_mean')(sampling_input)
        z_log_var = layers.Dense(sp1, name='z_log_var')(sampling_input)
        z = Sampling()([z_mean, z_log_var])
        return keras.Model(sampling_input, z, name='sampler')
    ...
    def model_builder(hp):
        ...
        sp1 = hp.Choice('sampling', values=[64, 128, 256])
        ...
        prior_weight = hp.Choice('l2-reg', values=[1.0, 0.3, 0.1])
        ...
        smp_img = vae_sampling(sp1)(agu_img)
        outputs = model_builder_colon(hp1, hp2, hp3, reg, num_layers)(smp_img)
        ...
        model.add_loss(prior_weight * tf.reduce_mean(tf.square(smp_img)))
        loss = custom_loss()
